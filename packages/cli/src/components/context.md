...
- `cliApp/useCommandLog.ts` now delegates payload cloning and inspector parsing to `cliApp/commandLogHelpers.ts`, keeping the hook small while the slash command continues to report how many entries are shown and clamps invalid counts before updating state.
