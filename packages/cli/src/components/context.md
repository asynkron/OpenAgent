...
- `DebugPanel.tsx`, `ThinkingIndicator.tsx` — optional diagnostics and spinner overlays; the debug panel renders payloads via the shared markdown renderer using `json` code fences so debug data is syntax-highlighted. `useDebugPanel` now watches for `__openagentStreamAction` payloads so structured response previews can stream through the panel and disappear once the runtime signals completion while reusing runtime-provided entry ids so replace/remove instructions only touch the targeted debug row.
