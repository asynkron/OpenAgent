...
- `DebugPanel.tsx`, `ThinkingIndicator.tsx` — optional diagnostics and spinner overlays; the debug panel renders payloads via the shared markdown renderer using `json` code fences so debug data is syntax-highlighted. `useDebugPanel` now watches for `__openagentStreamAction` payloads so structured response previews can stream through the panel and disappear once the runtime signals completion, and it funnels all bounded list updates through `cliApp/debugPanelManager.ts` so replacements/removals stay predictable.
