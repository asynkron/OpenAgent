...
- `cliApp/commandLogHelpers.ts` collects the cloning and slash-command inspector helpers shared by `useCommandLog` so the hook stays focused on orchestrating state updates.
