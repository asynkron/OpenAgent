...
- `CliApp.tsx` — top-level Ink tree that wires agent runtime events into UI sections and clears completed plans from the timeline once the human submits a fresh prompt. Auto-response debug payloads (schema/protocol validation failures) now surface as warn-level timeline entries so humans get a concise summary without enabling `--debug`. Assistant responses queue until command execution settles so the timeline shows commands before the summarised reply, and the handler now preserves structured assistant payloads instead of forcing them to empty strings so the timeline never drops a valid response row. The assistant message handler now asserts the runtime continues to emit string `__id` values so protocol regressions trigger a fast TypeError instead of silently corrupting the timeline. A lightweight `planning` runtime event now toggles a “planning…” indicator in the plan panel while the assistant streams partial steps, while final `plan` events still drive the rendered plan tree. The AskHuman input is memoised so caret visibility changes stay local without cascading into unnecessary rerenders of the rest of the CLI whenever other state updates land.
  Incoming runtime payloads are cloned before being stored so late mutations inside the agent loop can’t stall Ink re-renders. Timeline rows render strictly in arrival order (no `<Static>` buckets), so human prompts, executed commands, and assistant replies stay interleaved chronologically even as the timeline grows. Runtime-provided `__id` values key assistant/command entries, avoiding duplicate replacements across passes. The file delegates slash-command parsing and timeline rendering to `cliApp/slashCommands.ts` and `cliApp/Timeline.tsx`, keeping the component focused on state orchestration with idiomatic TypeScript types.
...
- `Command.tsx`, `commandUtils.ts` — pretty-print shell commands with highlights and approval status. Command rows now render as compact “Task” headers only: the macOS-style chrome shows the originating plan step context plus a status emoji. We intentionally omit run previews, exit payloads, and inline command text in the timeline so the stream stays focused on when tasks start and finish. Supporting pieces that used to live inline now reside under `command/` (theme cloning, plan headings, summary builders) so the main component stays lean.
