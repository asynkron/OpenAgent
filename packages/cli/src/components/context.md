...
- Display-focused components (plan/command/status renderers) now compile with TypeScript checks, reducing reliance on `@ts-nocheck` to the remaining interactive shells (`CliApp`, `AskHuman`). `InkTextArea` now participates fully in the type system, validating slash command hooks and caret math at compile time.
