...
import { formatDebugPayload, summarizeAutoResponseDebug } from './logging.js';
import {
  parseManagedDebugPayload,
  updateDebugEvents,
} from './debugPanelState.js';
...
export function useDebugPanel({ limit, appendStatus }: UseDebugPanelOptions): {
  ...
  const handleDebugEvent = useCallback(
    (event: DebugRuntimeEvent) => {
      const managedInstruction = parseManagedDebugPayload(event.payload);
      setDebugEvents((previous) =>
        updateDebugEvents({
          ...
          managedInstruction,
          limit,
          createEntry: createDebugEntry,
        }),
      );

      const summary =
        managedInstruction?.action === 'remove'
          ? null
