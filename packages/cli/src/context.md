...
- `runner.ts` & `runtime.ts` — orchestrate CLI startup, validate required environment configuration (e.g., `OPENAI_API_KEY`), normalize the agent runtime dependencies with TypeScript-aware guards, and pipe events into Ink with typed IO utilities.
