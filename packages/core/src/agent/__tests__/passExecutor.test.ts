...
describe('executeAgentPass', () => {
  ...
  test('clears completed plans when no steps remain open', async () => {
    ...
    const planManager = createPlanManager();
    planManager.update.mockResolvedValue([{ step: '1', title: 'Wrap up', status: 'completed' }]);
    planManager.reset.mockResolvedValue([]);
    ...
    expect(executeAgentCommand).not.toHaveBeenCalled();
    expect(planManager.update).toHaveBeenCalledTimes(1);
    expect(planManager.reset).toHaveBeenCalledTimes(1);
      ...
      plan: [
        {
          ...
          title: 'First',
          status: 'pending',
          command: { run: 'echo first' },
        },
        {
          step: '2',
          title: 'Second',
          status: 'pending',
          command: { run: 'echo second' },
        },
      ],
    };
