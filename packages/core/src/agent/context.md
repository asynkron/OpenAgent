...
- `passExecutor/planTypes.ts` centralizes the runtime-facing plan entry shape by extending the canonical tool schema with execution-time affordances (nullable commands, relaxed identifiers, optional priority). A shared `parseToolPlanResponse` helper now wraps the Zod schema so utilities work exclusively with typed plan DTOs, and co-located factories (`createPlanSnapshotStep`, `createPlanCommand`) keep tests from reintroducing `as unknown` scaffolding.
...
- `modelRequest.ts` (emits `modelRequest.js`) — wraps the AI SDK responses client with ESC cancellation support, emits cancellation observations when humans abort requests, and now consumes the shared response DTOs from `src/contracts` instead of importing types from the OpenAI adapter.
