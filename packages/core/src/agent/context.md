...
- `passExecutor/commandRuntime.ts` composes the approval, safe execution, stats tracking, and result emission helpers while emitting the active plan step snapshot alongside each `command-result` event so downstream UIs can display the parent step metadata with command output.
