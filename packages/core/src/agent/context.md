...
- `passExecutor/planTypes.ts` centralizes the runtime-facing plan entry shape by extending the canonical tool schema with execution-time affordances (nullable commands, relaxed identifiers, optional priority). Execution helpers and tests now import this module so schema adjustments only change a single source file.
