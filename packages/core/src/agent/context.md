...
- `observationBuilder.ts` (emits `observationBuilder.js`) — formats command results into preview payloads and observation envelopes while guarding against oversized outputs. It now enforces a 200-line default cap (with optional `command.tail_lines` overrides), honours an optional `command.max_bytes` limit, and annotates truncated output with explicit notices for the model.
