...
- `historyEntry.ts`, `historyCompactor.ts`, and `runtimePayloadGuard.ts` normalize chat history into the AI SDK message shape so downstream helpers stay provider-agnostic. `historyEntry.ts` now stores `role`/`content` exclusively inside the nested payload (with non-enumerable accessors for callers) and relies on the root chat-history contract, eliminating the duplicated fields that previously bloated history snapshots.
