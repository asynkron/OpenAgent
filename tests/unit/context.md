...
- CLI/UI: `renderCommand.test.js`, `renderPlan.test.js`, `renderLanguage.test.js`, `CliApp` component tests, `inkTextArea.test.js` for keyboard handling/terminal resize behavior, and `askHuman.test.js` to ensure the slash menu suggestions stay wired to default shortcuts. The Ink text area specs now also assert that multi-token slash queries exclude generic description-only matches so humans only see narrowed suggestions (e.g., `/model gpt-4o`).
