...
describe('HistoryCompactor', () => {
  ...
  test('compacts oldest entries when usage exceeds threshold', async () => {
    ...
    const [[payload]] = responsesCreate.mock.calls;
    expect(payload.model).toBe('test-model');
    const { input } = payload;
    expect(Array.isArray(input)).toBe(true);
    expect(input).toHaveLength(2);
    expect(input[1].content).toContain('Summarize the following 2 conversation entries');
...
y.slice(3).map((entry) => entry);
    ...
    const [{ model, input }] = responsesCreate.mock.calls;
